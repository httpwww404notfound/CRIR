...
class UserModel(nn.Module):
    def __init__(self, device, instance_dimesion=88, seed_dimesion = 128, n_hidden = 128, learning_rate=0.001):
    ...
    def softmax_feature(self, x):
        ...
        features[10] = x[:, 85:88]
        entropy = 0
        softmax_feature = FLOAT([]).to(self.device)
        for i in range(11):
    ...
    def generate(self, z = None):
        if z == None:
            z = torch.rand((1, self.seed_dimesion), device=self.device)
        ...
        one_hot = FLOAT()
        for i in range(11):
            tmp = torch.zeros(features[i].shape, device=self.device)
            one_hot = torch.cat((one_hot.to(self.device), tmp.scatter_(1, torch.multinomial(features[i], 1), 1)), dim = -1)
        return one_hot
